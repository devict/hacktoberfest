    ...
    .then(function(data) {
      ...
      data.forEach(function(issue) {
        ...
        for (var tag in issue["Labels"]) {
          tags += '<span class="inline-flex items-center px-2 rounded-full text-xs font-medium text-gray-900" style="background-color: #' + issue["Labels"][tag] + '">' + tag + '</span>';
        }

        rows += "<tr>" +
          '<td class="px-6 py-3 w-full text-sm font-medium text-gray-900"><a href="' + issue["URL"] + '">' + issue["Title"] + '</a><div>' + tags + '</div></td>' +
          '<td class="px-6 py-3 whitespace-nowrap text-sm font-medium text-gray-500">' + issue["Repo"]["Owner"] + "/" + issue["Repo"]["Name"] + "</td>";

        // some repos have no languages
        if (issue["Languages"] == null) {
          rows += '<td class="px-6 py-3 whitespace-nowrap text-sm font-medium text-gray-500">None</td></tr>';
        } else {
          rows += '<td class="px-6 py-3 whitespace-nowrap text-sm font-medium text-gray-500">' + issue["Languages"].join(", ") + "</td></tr>";
        }
      })
      $("#issues").append(rows);
      $('#issues_table').show();
